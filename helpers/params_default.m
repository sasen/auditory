function [P] = params_default(origFileName,outDirName,synth_dur,orig_dur)
% function [P] = params_default(origFileName,outDirName,synth_dur)
% returns the synthesis parameters struct for McDermott & Simoncelli 2011
%
...
P.project_root = '~/Dropbox/ucsd/projects/auditory/expts/'; %slash at end
P.orig_sound_folder = [P.project_root,'audio/library/']; %must be a string, should have a slash at the end. If this is an empty string, Matlab will search its path for the file.
P.orig_sound_filename = origFileName; %'Bubbling_water.wav'; % within P.orig_sound_folder
P.output_folder = [P.project_root,'audio/STIMULI/',outDirName,'/',]; %must be a string, should have a slash at the end. 
...
P.iteration_snapshots=[2 4 16]; %will save synthetic signal to file before specified iteration numbers
...
P.max_orig_dur_s = orig_dur;
P.desired_synth_dur_s = synth_dur;
